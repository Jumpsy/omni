...
12. `STRESS`: run broad quality validation, including many independent AI critiques and a scale-minded production-readiness pass.
...
- For major product work, run a large independent-review pass: simulate up to 100 separate AI-style checks when practical, or as many as the environment can support, and collect the failures into the critique log.
- Include a scale readiness check that asks whether the app could hold up under a 10,000-user load scenario; if real load testing is not available, document the assumption and use proxy evidence rather than claiming the result.
- Apply an Apple-like production bar: polished details, consistent interaction quality, reliable state handling, conservative motion, no obvious rough edges, and no unverified claims of production readiness.
- Never claim to have used Apple's internal testing process. Use "Apple-like" as a quality target only.
